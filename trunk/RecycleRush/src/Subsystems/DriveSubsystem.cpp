...
static const std::string& RotationDirectionToString(RotateDirection direction){
	switch (direction) {
...
DriveSubsystem::DriveSubsystem() : Subsystem("DriveSubsystem") {
	...
	autoHeading = DriveForward;
	autoRotateDirection = RotateRight;
	autoRotateInputAngle = 0.0;
	autoGyroTargetAngle = 0.0;
...
{
	if ((autoMode == Driving) && !autoActionInProgress) {
		autoActionInProgress = true;
...
bool DriveSubsystem::AutoRotateSetup(float angle, RotateDirection direction){
	if (!InRangeInclusive(angle,0,180)) {
		Logger::GetInstance()->Log(DriveSubsystemLogId,Logger::kERROR,"Rotation angle outside of range, angle = %f \n", angle);
		return false;
	}

	autoRotateInputAngle = angle;
	autoRotateDirection = direction;
	autoGyroTargetAngle = autoRotateInputAngle;

	if (autoRotateDirection == RotateLeft) {
		autoGyroTargetAngle += 180;
	}
...
//this is all just an idea it needs to be broken up to work better with a command
const float AutoRotateSpeed = 1.0;
...
void DriveSubsystem::AutoRotateExecute()
{
	...
	if ((autoMode == Rotating) && !autoActionInProgress) {
		autoActionInProgress = true;

		// here is where we would start rotation
		// Reset the gyro.
		gyro1->Reset();
		...
		{
			case RotateLeft:
			robotDrive->SetLeftRightMotorOutputs(-AutoRotateSpeed, AutoRotateSpeed);
			break;

			case RotateRight:
			robotDrive->SetLeftRightMotorOutputs(AutoRotateSpeed, -AutoRotateSpeed); //i would implement a system to slow it down when it gets near the destination angle
			break;
...
const float AngleDecimalPlacesPrecision = 100.00;
const int FullGyroSweep = 360 * static_cast<int>(AngleDecimalPlacesPrecision);

bool DriveSubsystem::AutoHasFinishedRotating() {

	if ((autoMode == Rotating) && autoActionInProgress)
	{
		float currGyroAngle = gyro1->GetAngle();

		currGyroAngle *= AngleDecimalPlacesPrecision;
		currGyroAngle = static_cast<float>((static_cast<int>(currGyroAngle) % FullGyroSweep)) / AngleDecimalPlacesPrecision;

		if (currGyroAngle >= autoGyroTargetAngle)
		{
			robotDrive->SetLeftRightMotorOutputs(0, 0);
			autoMode = Off;
			autoActionInProgress = false;
